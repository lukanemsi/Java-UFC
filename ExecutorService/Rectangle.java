...
public class Rectangle extends Figure implements Runnable
{
    ...
    public static Callable<LinkedList<Rectangle>> readRectangles(String path)
    {
        Callable<LinkedList<Rectangle>> rectangleCallable = () ->
        {
        ...
        Runnable fillRec = () ->
        {
            try(BufferedWriter writer = new BufferedWriter(new FileWriter(path)))
            {
                for (int i = 0; i < 70_000; i++) {
                    ...
                    writer.write((random.nextInt(10_000) + 1) + "-" + (random.nextInt(10_000) + 1) + '\n');
                }
            }catch (IOException e)
            {
                e.printStackTrace();
    ...
    public static LinkedList<Rectangle> getRectangles() {
        return rectangles;
    }
